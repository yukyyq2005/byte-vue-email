<template>
    ...
    <div v-show="!isShowLimitExceed">
        <!-- <div class="flex justify-between w-full px-4 py-3 text-sm font-medium text-left text-gray-900 bg-gray-200/50 focus:outline-none"><div class="space-x-2 flex flex-row py-0.5"><span class="w-5"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon"><path stroke-linecap="round" stroke-linejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244"></path></svg></span><p class="max-w-2xl truncate">Webhooks</p></div></div> -->
        <div class="px-6 py-6"><div class="bg-white">
            <!-- 中间控件 begin -->
            <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6"><p class="text-base font-semibold text-gray-900">{{ webhookStr }}</p><p class="text-sm leading-5 text-gray-500">{{ webhookDesc }}</p></div>
                
                <div class="col-span-4"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">{{webhookName}}</label></div><div class="mt-1"><input v-model="stored.notification_name" name="Rule Name" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div></div>
                <div class="col-span-4"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">{{webhookReqUrl}}</label></div><div class="mt-1"><input v-model="stored.pdf_file_name" name="url" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div></div>
                <div class="col-span-5">
                    <label for="m3jq3msr" class="block text-sm font-medium text-gray-700">{{ webhookBody }}</label>
...
const webhookDesc = ref('')
const webhookName = ref('')
const webhookReqUrl = ref('')
const webhookBody = ref('')
const cancelStr = ref('')
const saveContinueStr = ref('')

function setLocalizing(){
    webhookStr.value = translate('Webhooks for Forms')
    webhookDesc.value = translate('Automatically send Google Form responses to any webhook URL.')
    webhookName.value = translate('Webhook Name')
    webhookReqUrl.value = translate('Request URL')
    webhookBody.value = translate('Request Body')
...
async function saveButtonClick(params){
    ...
    if(!stored.pdf_file_name){
        resendToast.value.show('Please provide a URL')
        return
    }
