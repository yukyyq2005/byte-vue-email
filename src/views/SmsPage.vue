<template>
    ...
    <div v-show="!isShowLimitExceed">
        <!-- <div class="flex justify-between w-full px-4 py-3 text-sm font-medium text-left text-gray-900 bg-gray-200/50 focus:outline-none"><div class="space-x-2 flex flex-row py-0.5"><span class="w-5"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon"><path stroke-linecap="round" stroke-linejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5"></path></svg></span><p class="max-w-2xl truncate">SMS Notifications</p></div></div> -->
        <div class="px-6 py-6"><div class="bg-white">
            <!-- 中间控件 begin -->
            <div class="grid grid-cols-6 gap-6">
                <div class="col-span-6"><p class="text-base font-semibold text-gray-900">{{ smsTitle }}</p><p class="text-sm leading-5 text-gray-500">{{ smsDesc }}</p></div>
                
                <div class="col-span-3"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">{{ workflowName }}</label></div><div class="mt-1">
                    <input v-model="stored.notification_name" name="Rule Name" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div></div><div class="col-span-3">&nbsp;</div>
                <div class="col-span-3"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">Twilio Account SID</label></div><div class="mt-1">
                    <input v-model="stored.sender_name" name="sid" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div></div>
                <div class="col-span-3"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">Twilio Auth Token</label></div><div class="mt-1">
                    <input v-model="stored.email_subject" name="token" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div></div>
                <div class="col-span-3"><div class="flex justify-between items-end"><label class="block text-sm font-medium text-gray-700">{{ senderPhone }}</label></div><div class="mt-1">
                    <input v-model="stored.email_body"  name="from" type="text" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 rounded-md" value=""></div>
                </div>
                
                <div class="col-span-3">
                    <div class="flex justify-between"><label for="to" class="block text-sm font-medium text-gray-700">{{ reciPhone }}</label></div>
                    <Popover class="relative shadow-sm rounded-md mt-1" data-headlessui-state="">
                        <PopoverButton @click="colorClick(1)" title="Add Variable Field" class="cursor-pointer absolute inset-y-0 right-1.5 flex items-center focus:outline-none" aria-expanded="false" data-headlessui-state="" id="headlessui-popover-button-:r2j:"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true" data-slot="icon" class="h-5 w-5 text-gray-300 hover:text-gray-400 transition duration-200"><path stroke-linecap="round" stroke-linejoin="round" d="M3 4.5h14.25M3 9h9.75M3 13.5h9.75m4.5-4.5v12m0 0-3.75-3.75M17.25 21 21 17.25"></path></svg></PopoverButton>
                        <input ref="myInput"  v-model="stored.pdf_file_name" id="to" name="to" type="text" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full text-sm border-gray-300 pr-8 rounded-md" value="">
                        <PopoverColor @colorBtnClick="colorBtnClick1" :colorArray="colorArray" />
                    </Popover>
                ...
                <div class="col-span-6">
                    <label for="m3ihm7ej" class="block text-sm font-medium text-gray-700">{{ smsBody }}</label>
            ...
            <div class="col-span-full">
                <div class="mt-8 border-t border-gray-200 "><div class="flex items-center mt-6"><div class="flex justify-items-start">
                    <a @click.prevent="cancelClick" title="Discard all changes" type="button" class="bg-white py-2 px-4 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 hover:bg-gray-50 focus:outline-none" href="/">Cancel</a>
...
function setLocalizing(){
    smsTitle.value = translate('SMS Notifications')
    smsDesc.value = translate('Automatically send a text (SMS) message for Google Form responses with Twilio')
    workflowName.value = translate('Workflow Name')
    senderPhone.value = translate("Sender's Phone Number")
    reciPhone.value = translate("Recipient's Phone Number")
    smsBody.value = translate('SMS Message Body')
}
onMounted(()=>{
    setLocalizing()
