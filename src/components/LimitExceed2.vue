<template>
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-sm leading-5 text-gray-500">You have reached the maximum number of simultaneous forms with "Form Limiter".</p>
            <p class="text-sm leading-5 text-gray-500">You can disable an unused "Form Limiter" for one of the forms below.</p>
        </div>
    ...
    <p v-if="!requestDataing" class="mt-6 text-sm text-gray-900">
        <ul class="divide-y divide-gray-100 rounded-md border border-gray-200">
            <li v-for="item in newTriggerList" :key="item.formId"
...
const newTriggerList = props.triggerList;
...
function toggleClick(item){
    ...
    if (isSwitch){
        ...
        if (item.formId){
            ...
            .closeLimiterSwitch(item.formId);
        }else if(item.uuid){
            google.script.run.withSuccessHandler(() => {
                handerSuccess()
            })
            ...
            .deleteLimiterTrigger(item.uuid, item.type);
        }
